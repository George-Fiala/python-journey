...
def show_summary(leaks):
    print("\n--- Leak Summary ---")
    if not leaks:
        print("No leaks recorded yet.")
        return

    total = len(leaks)
    open_leaks = sum(1 for l in leaks if l["status"] == "OPEN")
    closed_leaks = sum(1 for l in leaks if l["status"] == "CLOSED")
    ...
    for l in leaks:
        if l["status"] == "OPEN":
            ...
            # overdue?
            due_date_obj = datetime.fromisoformat(l["due_date"]).date()
            if due_date_obj < today:
                overdue_open += 1

    print(f"Total leaks:        {total}")
    print(f"Open leaks:         {open_leaks}")
    print(f"    HIGH open:      {high_open}")
    print(f"    MEDIUM open:    {med_open}")
    print(f"    LOW open:       {low_open}")
    print(f"Closed leaks:       {closed_leaks}")
    print(f"Overdue OPEN leaks: {overdue_open}")
